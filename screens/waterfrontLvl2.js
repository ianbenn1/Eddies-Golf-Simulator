...
import { StyleSheet, View, ImageBackground, TouchableWithoutFeedback, Text, Alert, Dimensions, TouchableOpacity } from 'react-native';
...
class WaterfrontLvl2 extends React.Component {
  constructor(props){
    super(props);
...
async componentDidMount() {
  ...
  levelsScore = JSON.parse(levelsScore).lvl2;
  ...
  setupWorld = () => {
    ...
    let ball = Matter.Bodies.rectangle((Dimensions.get("screen").width - 50), 75, 30, 30);
    ...
    let waterwall = Matter.Bodies.rectangle( Dimensions.get("screen").width-50, Dimensions.get("screen").height / 1.5 , Dimensions.get("screen").width/1.4, Dimensions.get("screen").height/1.4, { isStatic: true });

    Matter.World.add(world, [ball, waterwall, floor, ceiling, leftwall, rightwall]);
      ...
      if(ballYCoords > 850 && ballXCoords > 100 && ballXCoords < 110)
      {
        if(this.state.shown == 0)
        {
          ...
          if(this.state.score < levelsScore || levelsScore == 0)
          {
            ...
            today = yyyy + "-" + mm + "-" + dd;
            createDataFiles(FileSystem.documentDirectory + "gamesettings" + "/score.txt", '{"lvl1": '+this.state.lvl1score+', "lvl1Time": "'+this.state.lvl1date+'", "lvl2": '+this.state.score+', "lvl2Time": "'+today+'", "lvl3": '+this.state.lvl3score+', "lvl3Time": "'+this.state.lvl3date+'", "lvl4": '+this.state.lvl4score+', "lvl4Time": "'+this.state.lvl4date+'"}');
    ...
    return {
        ...
        waterwall: { body: waterwall, size: [Dimensions.get("screen").width/1.4, Dimensions.get("screen").height/1.4], color: this.state.maskColour, renderer: Wall },
    }
}

  render() {
  return (
    <ImageBackground source={require('../assets/levels/lvl2.png')} style={gestyles.container}>
      <GameEngine
